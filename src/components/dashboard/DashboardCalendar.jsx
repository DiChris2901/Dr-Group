...
const calculateTenthBusinessDay = (year, month, holidays) => {
  ...
  // Convertir festivos a array de fechas para comparación más fácil
  ...
  while (businessDayCount < 10) {
    ...
    const isHoliday = holidayDates.some(holidayDate => 
      holidayDate.getTime() === currentDayNormalized.getTime()
    );
    
    // Si no es fin de semana ni festivo, cuenta como día hábil
    if (!isWeekendDay && !isHoliday) {
      businessDayCount++;
    }
    
    // Si ya llegamos al décimo día hábil, retornar esta fecha
    ...
    // Protección contra bucle infinito (no debería pasar, pero por seguridad)
    if (currentDay.getMonth() !== month) {
      console.warn('⚠️ No se pudo encontrar el décimo día hábil en el mes', month + 1, year);
      return null;
    }
  }
