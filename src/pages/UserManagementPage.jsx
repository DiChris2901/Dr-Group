      ...
      if (editingUser) {
        ...
        try {
          ...
          const simpleUserData = {
            ...
            permissions: (() => {
              const permObj = {};
              formData.permissions.forEach(p => { permObj[p] = true; });
              return permObj;
            })(),
            companies: formData.companies,
            isActive: true,
      ...
      >
        ...
        <DialogContent sx={{ p: 0 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', height: 'calc(100% - 64px)' }}>
            ...
            {/* TAB 2: PERMISOS DE ACCESO - TABLA HÍBRIDA */}
            {activeTab === 1 && (
              <Box sx={{ p: 3, overflowY: 'auto', flexGrow: 1 }}>
                {(() => {
                  // Función helper para renderizar una fila de permiso
                  const renderPermissionRow = (perm, level = 1, parentColor, parentIsActive = false, parentPerm = null) => {
                    const color = perm.color || parentColor;
                    const isDirectlyActive = formData.permissions.includes(perm.key);
                    // No heredar estado del padre si tiene noAutoActivate (ej: solicitudes.gestionar)
                    const isActive = isDirectlyActive || (parentIsActive && !perm.noAutoActivate);
                    const paddingLeft = level === 1 ? 2 : level === 2 ? 5 : 8;
                    
                    // Calcular contador de sub-permisos activos
                    let badge = null;
                    if (perm.subPermissions && level === 1) {
                      const totalCount = perm.subPermissions.length;
                      
                      // Si el padre está activo, TODOS los sub-permisos están activos
                      if (isActive) {
                        badge = `${totalCount}/${totalCount}`;
                      } else {
                        // Si el padre NO está activo, contar cuántos sub-permisos individuales están activos
                        const activeCount = perm.subPermissions.filter(sp => 
                    ...
                    const handleToggle = (e) => {
                      ...
                      if (parentIsActive && parentPerm) {
                        // Caso especial: el padre está activo y el usuario interactúa con un hijo
                        // → Desactivar el padre, expandir a permisos individuales
                        newPermissions = formData.permissions.filter(p => p !== parentPerm.key);
                        
                        if (isActive) {
                          // El hijo ESTABA activo → usuario quiere DESACTIVARLO
                          // Activar todos los hermanos EXCEPTO este (respetando noAutoActivate y exclusividad)
                          parentPerm.subPermissions.forEach(sibling => {
                            if (sibling.key !== perm.key && !newPermissions.includes(sibling.key)) {
                              if (!sibling.noAutoActivate) {
                                newPermissions.push(sibling.key);
                              }
                            }
                          });
                        } else {
                          // El hijo NO estaba activo (noAutoActivate) → usuario quiere ACTIVARLO
                          // Activar todos los hermanos que estaban activos + este, respetando exclusividad mutua
                          parentPerm.subPermissions.forEach(sibling => {
                            if (!newPermissions.includes(sibling.key)) {
                              if (!sibling.noAutoActivate) {
                                newPermissions.push(sibling.key);
                              }
                            }
                          });
                          // Agregar el permiso clickeado
                          if (!newPermissions.includes(perm.key)) {
                            newPermissions.push(perm.key);
                          }
                          // Exclusividad mutua: quitar el permiso mutuamente excluyente
                          if (perm.mutuallyExclusiveWith) {
                            newPermissions = newPermissions.filter(p => p !== perm.mutuallyExclusiveWith);
                          }
                        }
                      ...
                      if (isDirectlyActive) {
                        // Desactivar
                        newPermissions = formData.permissions.filter(p => p !== perm.key);
                        
                        // Si tiene sub-permisos, también desactivarlos
                        if (perm.subPermissions) {
                          perm.subPermissions.forEach(sp => {
                            newPermissions = newPermissions.filter(p => p !== sp.key);
                            // Si el sub-permiso tiene sub-sub-permisos
                            if (sp.subPermissions) {
                              sp.subPermissions.forEach(ssp => {
                                newPermissions = newPermissions.filter(p => p !== ssp.key);
                              });
                            }
                          });
                        }
                        ...
                        // Activar
                        newPermissions = [...formData.permissions, perm.key];
                        
                        // Exclusividad mutua directa (ej: solicitudes ↔ solicitudes.gestionar)
                        if (perm.mutuallyExclusiveWith) {
                          newPermissions = newPermissions.filter(p => p !== perm.mutuallyExclusiveWith);
                        }
                        
                        // Si tiene sub-permisos, activarlos (respetando noAutoActivate)
                        if (perm.subPermissions) {
                          perm.subPermissions.forEach(sp => {
                            if (!sp.noAutoActivate && !newPermissions.includes(sp.key)) {
                              newPermissions.push(sp.key);
                              
                              // Respetar exclusividad mutua
                              if (sp.mutuallyExclusiveWith) {
                                newPermissions = newPermissions.filter(p => p !== sp.mutuallyExclusiveWith);
                              }
                            }
                          });
                        }
                      ...
                      <React.Fragment key={perm.key}>
                        <Box 
                          sx={{ 
                            ...
                            pl: paddingLeft,
                            cursor: 'pointer',
                            backgroundColor: level === 1 && isActive ? alpha(color, 0.04) : 'transparent',
                            borderBottom: level === 1 ? `1px solid ${alpha(theme.palette.divider, 0.08)}` : 'none',
                            '&:hover': {
                        ...
                        >
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flex: 1 }}>
                            ...
                            <Typography variant="body2" sx={{ 
                              ...
                              fontSize: level === 3 ? '0.8rem' : '0.875rem'
                            }}>
                              {perm.label}
                            </Typography>
                        ...
                        {/* Renderizar sub-permisos */}
                        {perm.subPermissions && perm.subPermissions.map(subPerm => renderPermissionRow(subPerm, level + 1, color, isActive, perm))}
                      </React.Fragment>
                    );
                  };

                  // Dividir permisos en dos columnas
                  const leftColumnPerms = [
                    ...
                    { 
                      ...
                      subPermissions: [
                        ...
                        { key: 'liquidaciones.estadisticas', label: 'Estadísticas' }
                      ]
                    },
                    { 
                      key: 'facturacion', 
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12} md={6}>
                        <Box sx={{ 
                          ...
                          <Box>
                            {leftColumnPerms.map(perm => renderPermissionRow(perm))}
                          </Box>
                        </Box>
                      </Grid>
                      ...
                      <Grid item xs={12} md={6}>
                        <Box sx={{ 
                          ...
                          <Box>
                            {rightColumnPerms.map(perm => renderPermissionRow(perm))}
                          </Box>
                        </Box>
                      </Grid>
                    </Grid>
